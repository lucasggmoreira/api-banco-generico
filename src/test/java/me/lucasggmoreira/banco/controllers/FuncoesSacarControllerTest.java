...
@Transactional
class FuncoesSacarControllerTest {
    ...
    @Test
    @DisplayName("Deve retornar o código 400 ao tentar sacar sem informar o valor")
    public void transferencia_1() throws Exception {
        var request = mvc.perform(post("/funcoes/sacar")
    ...
    @Test
    @DisplayName("Deve retornar o código 400 ao tentar sacar zero")
    ...
    @Test
    @DisplayName("Deve retornar o código 400 ao tentar sacar um valor negativo")
    public void transferencia_3() throws Exception {
        var request = mvc.perform(post("/funcoes/sacar").
                contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("Deve retornar o código 400 ao tentar sacar sem ter saldo suficiente")
    public void transferencia_4() throws Exception {
        var request = mvc.perform(post("/funcoes/sacar").
                contentType(MediaType.APPLICATION_JSON)
                .content(dadosValorTransferencia.write(new DadosValorTransferencia(200)).getJson())
                .header("Authorization", "Bearer " + token)).andReturn().getResponse();
        assertThat(request.getStatus()).isEqualTo(HttpStatus.BAD_REQUEST.value());
    }
    ...
    @Test
    @DisplayName("Deve retornar o código 200 ao sacar com sucesso")
    ...
    private void depositar(double valor) throws Exception {
        mvc.perform(post("/funcoes/depositar").
                contentType(MediaType.APPLICATION_JSON)
                .content(dadosValorTransferencia.write(new DadosValorTransferencia(valor)).getJson())
                .header("Authorization", "Bearer " + token));
    }
}